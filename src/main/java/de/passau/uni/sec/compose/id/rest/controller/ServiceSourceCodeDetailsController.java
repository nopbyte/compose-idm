...
public class ServiceSourceCodeDetailsController {
private static Logger LOG = LoggerFactory.getLogger(ServiceSourceCodeDetailsController.class);
	
	@Autowired
    private ServiceSourceCodeService serviceSourceCodeService;
    ...
    public @ResponseBody ResponseEntity<Object> getUser( @RequestHeader("Authorization") String token,
    		@PathVariable(value="Id") String uid,UriComponentsBuilder builder){
    			...
    			try{
		    		 ...
		    		 EntityResponseMessage res = serviceSourceCodeService.getEntity(new GetServiceSourceCodeEvent(uid,principals));
