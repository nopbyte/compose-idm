...
private static Logger LOG = LoggerFactory.getLogger(ListAllGroupsController.class);
	...
	@Autowired
	private ListGroupService service;
    ...
    public @ResponseBody ResponseEntity<Object> getMembershipsUser(   @RequestHeader("Authorization") String token,
			...
			cred.add(token);
		    	try{
    				 ...
    				 ListGroupsEvent event = new ListGroupsEvent(principals,page);
    				 List<GroupResponseMessage> res=(List<GroupResponseMessage>) service.listAllEntities(event);
